...
#define PROC_MEMSHARE_SIZE "button_code"
...
extern int gpio_keys_yaomoon_test(void);

static int proc_read_phymem_addr(char *page, char **start, off_t off, int count)
{
        return sprintf(page, "%08lx\n", __pa(kernel_memaddr));
}
static int proc_read_button_code(char *page, char **start, off_t off, int count)
{
        return sprintf(page, "%d", gpio_keys_yaomoon_test());
}
 ...
 static int __init init(void)
 {
         /*build proc dir "memshare"and two proc files: phymem_addr, phymem_size in the dir*/
          ...
          create_proc_read_entry(PROC_MEMSHARE_SIZE, 0, proc_memshare_dir, proc_read_button_code,NULL);
